...
export default function PuntenOverzicht({
  matches,
  predRows,
  jokerRows,
  advancementRows,
  ...
  const tabs: { id: Tab; label: string; pts: number | null }[] = [
    { id: 'groepsfase', label: 'Wedstrijden', pts: groepsPts || null },
    { id: 'eindstand', label: 'Wie door', pts: null },
    { id: 'knockout', label: 'Knockout', pts: koPts || null },
    { id: 'bonus', label: 'Bonusvragen', pts: bonusPts || null },
  ]
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="flex gap-1 overflow-x-auto scrollbar-none bg-wk-surface/80 border border-white/10 rounded-full p-1 backdrop-blur-sm">
        {tabs.map((t) => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
          ...
          >
            ...
            {t.pts != null && (
              <span
                className={`text-[9px] px-1.5 py-0.5 rounded-full border transition-colors duration-200 ${
                  tab === t.id
      ...
      <div key={tab} className="animate-fade-up space-y-5">
        ...
        {tab === 'groepsfase' && (
          <div className="space-y-5">
            {matches.length === 0 ? (
              <EmptyState>Nog geen wedstrijden</EmptyState>
            ) : (
              GROUPS.map((group, gi) => {
...
function PtsBadge({ pts }: { pts: number }) {
  return (
    <span
      className={`font-mono text-[10px] font-bold px-2 py-0.5 rounded-full border tracking-widest shrink-0 ${
        pts > 0
