...
type Enemy = { x: number; y: number; vy: number; type: 'walk' | 'fly'; hp: number; frame: number; ft: number; grounded: boolean; fireCd: number; bob: number }
type Boom = { x: number; y: number; frame: number; ft: number; scale: number }
type Tank = { x: number; y: number; hp: number; max: number; fireCd: number; frame: number; ft: number; vx: number }
...
export default function GunClient({ leaderboard, currentUserId }: { leaderboard: LeaderEntry[]; currentUserId: string }) {
  ...
  const [hud, setHud] = useState({ score: 0, hp: 3, bossHp: 0, bossMax: 0, progress: 0 })
  ...
  const reset = useCallback(() => {
    ...
    bullets.current = []; ebullets.current = []; enemies.current = []; booms.current = []; tank.current = null
    camX.current = 0; score.current = 0; spawnCd.current = 1.4; bossOn.current = false
    inL.current = false; inR.current = false
    setHud({ score: 0, hp: 3, bossHp: 0, bossMax: 0, progress: 0 })
  }, [])
  ...
  const loop = useCallback((t: number) => {
    ...
    // camera
    camX.current = Math.max(0, Math.min(LEN - W, player.current.x - 180))
  ...
  const step = (dt: number) => {
    ...
    // vijand-spawns (tot de baas-zone)
    if (!bossOn.current && p.x < LEN - 620) {
      spawnCd.current -= dt
      if (spawnCd.current <= 0 && enemies.current.length < 9) {
        const fly = Math.random() < 0.4
        const sx = camX.current + W + 30
        if (fly) enemies.current.push({ x: sx, y: 90 + Math.random() * 80, vy: 0, type: 'fly', hp: 2, frame: 0, ft: 0, grounded: false, fireCd: 1.5, bob: Math.random() * 6 })
    ...
    if (!bossOn.current && p.x > LEN - 620) {
      bossOn.current = true
      tank.current = { x: LEN - 140, y: GROUND, hp: 70, max: 70, fireCd: 1.2, frame: 0, ft: 0, vx: -34 }
      setHud((h) => ({ ...h, bossHp: 70, bossMax: 70 }))
    }

    // vijanden updaten
    for (const e of enemies.current) {
      e.ft += dt; if (e.ft > 0.1) { e.ft = 0; e.frame++ }
      if (e.type === 'walk') { e.x += Math.sign(p.x - e.x) * 64 * dt; e.y = GROUND }
      else {
    ...
    if (tk) {
      tk.ft += dt; if (tk.ft > 0.16) { tk.ft = 0; tk.frame = (tk.frame + 1) % TANK.length }
      tk.x += tk.vx * dt; if (tk.x < LEN - 220 || tk.x > LEN - 80) tk.vx *= -1
      tk.fireCd -= dt
      if (tk.fireCd <= 0) {
        const rage = tk.hp < tk.max * 0.5
        tk.fireCd = rage ? 0.95 : 1.4
        for (const off of rage ? [-0.25, 0, 0.25] : [-0.18, 0.18]) {
          const a = Math.atan2((p.y - 30) - (tk.y - 30), p.x - tk.x) + off
          ebullets.current.push({ x: tk.x - 30, y: tk.y - 30, vx: Math.cos(a) * 250, vy: Math.sin(a) * 250 - 60, grav: true })
        }
      }
    }
    ...
    for (const b of bullets.current) {
      for (const e of enemies.current) {
        if (e.hp <= 0) continue
        const rw = e.type === 'fly' ? 26 : 22, rh = e.type === 'fly' ? 22 : 30
        if (Math.abs(b.x - e.x) < rw && Math.abs(b.y - (e.y - rh / 2)) < rh) { e.hp -= 1; b.x = -9999; if (e.hp <= 0) { score.current += e.type === 'fly' ? 20 : 15; addBoom(e.x, e.y - 18, 0.8) } break }
      }
      if (tk && b.x !== -9999 && Math.abs(b.x - tk.x) < 60 && b.y > tk.y - 56 && b.y < tk.y) {
        tk.hp -= 1; b.x = -9999; if (Math.random() < 0.4) addBoom(b.x, b.y, 0.5)
        setHud((h) => ({ ...h, bossHp: Math.max(0, tk.hp) }))
        if (tk.hp <= 0) { for (let i = 0; i < 16; i++) setTimeout(() => addBoom(tk.x + (Math.random() - 0.5) * 130, tk.y - 30 + (Math.random() - 0.5) * 60, 1.4), i * 60); score.current += 1000; tank.current = null; setTimeout(() => endGame(true), 1100) }
      }
    }
    ...
    if (p.inv <= 0) {
      for (const e of enemies.current) { const rw = e.type === 'fly' ? 24 : 20; if (Math.abs(e.x - p.x) < rw && Math.abs((e.y - 18) - (p.y - 22)) < 32) { hitPlayer(); break } }
    }
  ...
  const drawWorld = (ctx: CanvasRenderingContext2D) => {
    ...
    for (const e of enemies.current) {
      const flip = p.x < e.x
      if (e.type === 'fly') drawImg(ctx, FLY[e.frame % FLY.length], e.x, e.y + 30, 40, flip)
      ...
      <div className="relative max-w-md mx-auto px-4 py-8 sm:py-12 space-y-5">
        ...
        {phase === 'playing' && (
          <div className="flex items-center justify-between bg-wk-surface border border-white/10 rounded-xl px-4 py-2">
            ...
            <div className="text-center"><p className="font-mono text-[9px] text-wk-muted tracking-[0.16em] uppercase">{hud.bossMax > 0 ? 'Tank' : `${hud.progress}%`}</p><p className="font-score text-lg leading-none text-wk-text">{'❤'.repeat(Math.max(0, hud.hp))}</p></div>
        ...
        <div className="relative mx-auto w-full max-w-[460px] select-none touch-none">
          ...
          {phase === 'playing' && hud.bossMax > 0 && (
            <div className="absolute top-2 left-3 right-3 z-10">
              <p className="font-mono text-[9px] text-wk-red tracking-[0.2em] uppercase text-center mb-1 drop-shadow">⚠ Alien-Tank</p>
          ...
          {phase === 'idle' && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-center px-6 bg-wk-bg/60 backdrop-blur-[1px] rounded-2xl">
              ...
              <p className="text-sm text-wk-soft leading-relaxed">
                <b className="text-wk-gold">◀ ▶</b> om te lopen, <b className="text-wk-gold">JUMP</b> om te springen — je <b>schiet automatisch</b> vooruit. Maai je door de aliens en versla de <b className="text-wk-red">Alien-Tank</b> aan het eind. <span className="text-wk-muted">(Desktop: pijltjes + spatie)</span>
              </p>
