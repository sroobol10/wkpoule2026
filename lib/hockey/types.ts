...
export type GameState = {
  ...
  // Vechtpartij (1v1) na een hele wilde check: het spel bevriest, twee man gaan op de vuist.
  // 'fight' = worstelen/meppen, 'ko' = de verliezer gaat neer; daarna tijdstraf + face-off.
  brawl: { slider: number; victim: number; behind: boolean; spot: Vec2; t: number; phase: 'fight' | 'ko'; winner: number } | null
  brawlCount: number // teller — de client toont er de "VECHTPARTIJ!"-toast op
  lastBrawlWinner: number // speler-id van de laatste brawl-winnaar (-1 = geen/nog bezig)
