...
// Formaties/lines (elk PLAYERS_PER_TEAM=6 plekken; index 0 = goalie).
// anchor genormaliseerd (x: 0 eigen doel → 1 tegenstander, y: 0 boven → 1 onder).
// Hockey-standaard: 2 defensemen + 3 forwards (LW/C/RW); varianten schuiven de balans.
export type FormationSlot = { role: Role; anchor: Vec2 }
export type Formation = { id: string; label: string; slots: FormationSlot[] }
export const FORMATIONS: Formation[] = [
  {
    id: '2-3', label: '2-3 (klassiek)',
    slots: [
      { role: 'GK', anchor: { x: 0.05, y: 0.5 } },
      { role: 'DEF', anchor: { x: 0.26, y: 0.32 } },
      { role: 'DEF', anchor: { x: 0.26, y: 0.68 } },
      { role: 'FWD', anchor: { x: 0.62, y: 0.2 } },
      { role: 'MID', anchor: { x: 0.55, y: 0.5 } },
      { role: 'FWD', anchor: { x: 0.62, y: 0.8 } },
    ],
  },
  {
    id: '3-2', label: '3-2 (defensief)',
    slots: [
      { role: 'GK', anchor: { x: 0.05, y: 0.5 } },
      { role: 'DEF', anchor: { x: 0.24, y: 0.25 } },
      { role: 'DEF', anchor: { x: 0.2, y: 0.5 } },
      { role: 'DEF', anchor: { x: 0.24, y: 0.75 } },
      { role: 'FWD', anchor: { x: 0.62, y: 0.35 } },
      { role: 'FWD', anchor: { x: 0.62, y: 0.65 } },
    ],
  },
  {
    id: '1-4', label: '1-4 (all-out attack)',
    slots: [
      { role: 'GK', anchor: { x: 0.05, y: 0.5 } },
      { role: 'DEF', anchor: { x: 0.22, y: 0.5 } },
      { role: 'MID', anchor: { x: 0.5, y: 0.3 } },
      { role: 'MID', anchor: { x: 0.5, y: 0.7 } },
      { role: 'FWD', anchor: { x: 0.68, y: 0.36 } },
      { role: 'FWD', anchor: { x: 0.68, y: 0.64 } },
    ],
  },
]
